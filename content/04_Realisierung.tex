...
In diesem Kapitel werden die praktischen Aspekte der Umsetzung dokumentiert.\footnote{Implementierungsdetails und Best Practices sind in \cite{doe2019research} dokumentiert.} Die Implementierung erfolgte iterativ über einen Zeitraum von 4 Monaten mit kontinuierlichem Testen und Refinement.
...
\begin{description}
  \item[Programmiersprache:] Python 3.11+ (Type Hints, async/await Support)
  \item[LLM-Integration:] OpenAI API (GPT-4), Anthropic API (Claude 3.5 Sonnet), mit Fallback-Mechanismus
  \item[Orchestrierung:] LangChain 0.1.x für Basis-Komponenten, custom Extensions für SE-specific Tools
  \item[Gedächtnis:] Chroma Vector-DB für semantisches Retrieval, SQLite für episodische Logs
  \item[Tool-Runtime:] Docker 24.0+ für Sandboxing, pytest für Tests, ruff/black für Linting
  \item[Monitoring:] Prometheus für Metriken, strukturierte Logs (JSON) mit Trace-IDs
\end{description}
...
\subsection{Kernimplementierung: Agent Controller}

Listing \ref{lst:agent-loop} zeigt einen minimalen agentischen Loop mit Planung, Tool-Ausführung und Reflexion. Die vollständige Implementierung umfasst zusätzlich Error-Handling, Timeouts, max-steps-Limits und strukturiertes Logging.
