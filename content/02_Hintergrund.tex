...
Large Language Models (LLMs) sind neuronale Netze, die auf großen Textkorpora trainiert wurden und menschenähnliche Textgenerierung ermöglichen \cite{OpenAI2024GPT4,touvron2023llama}. Basierend auf der Transformer-Architektur \cite{vaswani2017attention} nutzen sie Selbstaufmerksamkeits-Mechanismen (self-attention), um kontextuelle Zusammenhänge über lange Sequenzen hinweg zu erfassen.

Moderne LLMs wie GPT-4, Claude oder Llama 2 verfügen über Milliarden von Parametern und können durch \emph{Few-Shot-Learning} und \emph{In-Context-Learning} Aufgaben lösen, ohne explizit darauf trainiert zu werden. Dies bildet die Grundlage für agentische Anwendungen.
...
\begin{enumerate}
  \item \textbf{Zielgerichtetes Handeln:} Der Agent versteht übergeordnete Ziele und plant Schritte zur Zielerreichung
  \item \textbf{Werkzeugnutzung:} Integration externer Tools (APIs, Datenbanken, Code-Execution)
  \item \textbf{Gedächtnis:} Persistierung von Kontextinformationen über mehrere Interaktionen hinweg
  \item \textbf{Reflexion:} Selbstkritische Bewertung eigener Outputs und iterative Verbesserung
  \item \textbf{Mehrschrittige Planung:} Dekomposition komplexer Aufgaben in Teilschritte
\end{enumerate}
...
\begin{description}
  \item[Zustand (State):] Umfasst aktuellen Kontext, Ziele, Erinnerungen und Tool-Status. Der Zustand wird dynamisch aktualisiert.
  
  \item[Policy:] Steuert Entscheidungsprozesse (Planung, Aktion, Selbstkritik). Kann regelbasiert oder LLM-gesteuert sein.
  
  \item[Werkzeuge (Tools):] Externe Funktionen wie Code-Ausführung, Websuche, Dateizugriff, VCS-Operationen. Tools erweitern die Fähigkeiten des Agenten über reine Textgenerierung hinaus \cite{schick2023toolformer,qin2023tool}.
...
Orchestrierung koordiniert diese Komponenten durch Planung (z.\,B. ReAct-Pattern), Tool-Auswahl, Fehlerbehandlung und Reflexion \cite{yao2023react}.
