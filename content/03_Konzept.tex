...
\begin{sloppypar}
\begin{description}
  \item[Modularität:] Klare Trennung zwischen Policy-Logik, Tool-Adaptern, Gedächtnis und Safety-Layer. Komponenten kommunizieren über wohldefinierte Interfaces.\index{Modularität}
  ...
  \item[Robustheit:] Fehlertoleranz durch Retry-Logik, Timeouts, Circuit-Breakers. Graceful Degradation bei Teil-Ausfällen.\index{Retry}\index{Circuit Breaker}\index{Timeout}
...
\begin{description}
  ...
  \item[Tool-Registry:] Verwaltung verfügbarer Tools mit Metadaten (Name, Beschreibung, Schema, Permissions)\index{Tool-Metadaten}. Ermöglicht Tool-Erkennung\index{Tool-Discovery} und dynamisches Routing.
  
  \item[Tool-Adapter:] Wrapper für externe Tools (Tests, Linter, VCS)\index{Tool-Wrapper}. Standardisieren Input/Output-Formate\index{Standardisierung}. Implementieren Retry-Logik und Fehlerbehandlung\index{Error-Handling}.
  ...
  \item[Safety-Layer:] Interceptor für alle Tool-Calls. Prüft Berechtigungen, erzwingt Rate-Limits, loggt kritische Operationen. Kann schädliche Calls blockieren.
...
\begin{enumerate}
  ...
  \item \textbf{Observation:} Tool-Ausgabe wird strukturiert zurückgegeben. Beispiel: „3 tests failed in auth/test\_login.py"
...
\begin{sloppypar}
\begin{itemize}
  \item Automatische Tool-Erkennung und -Registrierung
...
\begin{enumerate}
  ...
  \item \textbf{Reflection:} Agent analysiert Fehlermeldungen, generiert einen präzisen Patch-Verfeinerungsvorschlag und wiederholt ggf. den Loop.
\end{enumerate}
